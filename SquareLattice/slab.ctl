...
(define-param a 0.5) ; Size of the square dielectric slab 
...
; x direction as well.
(set! geometry-lattice (make lattice (size 1 no-size Z)))
...
(set! geometry
      (list (make block ; a dielectric block (a rectangle)
	      ...
	      (size a infinity a) ; block is finite only in y direction
...
(set! k-points (interpolate k-interp 
			    (list (vector3 0 KY 0) (vector3 0.5 KY 0))))
...
;(run-tm output-efield-x)
;(run-tm output-efield-y)
